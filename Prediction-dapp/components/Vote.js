...
//const supabase = createClient('https://ymrnvwcpcnamolzisoej.supabase.co', process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
...
const Content = () => {
  ...
  console.log(`Current poll status: Ethereum=${votes.Amazon} | Solana=${votes.solana} | Tesla=${votes.Tesla}`);
  console.log(``);
  ...
  const fetchPoll = async(pollId) => {
    ...
    setVotes({
      Amazon: parseInt(pollAccount.Amazon.toString()),
      Google: parseInt(pollAccount.Google.toString()),
      Tesla: parseInt(pollAccount.Tesla.toString()),
    });
  ...
  return (
    <div className="App">
      ...
      {publicKey ? (
        <>
          <h1>Vote for speculative stocks/assets best performers</h1>
          ...
          <h3>Cast your speculative vote below</h3>
          ...
          <h2>Amazon: {votes.Amazon} | Google: {votes.Google} | Tesla: {votes.Tesla}</h2>
        </>
      ) : (
